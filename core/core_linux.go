...
func (s *Service) GenHost() error {
	...
	if err != nil {
		return fmt.Errorf("Failed to lookup address of '%v' - %v", s.Interface, err)
	}

	r, err := regexp.Compile(".*inet addr:([0-9]*.[0-9]*.[0-9]*.[0-9]*)")
	if err != nil {
		return fmt.Errorf("Regex compile fail - %v", err)
	}

	addr := r.FindStringSubmatch(fmt.Sprintf("%+v", string(ifout)))
	if len(addr) != 2 {
		return fmt.Errorf("No address found for '%v' - %v", s.Interface, err)
	}
